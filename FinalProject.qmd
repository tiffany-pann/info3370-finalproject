...
For this graph, we also needed to drop cases that were NA in our original data set. We dropped the NA cases in `wtsurvy`, `jobsatis`, and `ndgmemg`, since we are observing majors now. In addition, we also filter out cases where the `ndgmemg` variable was 99, since this indicates that the respondent skipped the question.

This brings our data from 1,140,565 to 139,656 cases.
...
We notice that our data set does not included specific data about non-stem fields. Hence, we acknowledge that this data set has some bias.
...
#### Further Exploration: Race and Ethnicity: 

We wanted to dive deeper into this topic, and hence introduce a new variable **raceth,** which indicates the individual's race/ethnicity:
